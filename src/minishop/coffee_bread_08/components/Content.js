...
function App(props) {
    ...
    function getProductList(item, index) {
        ...
        const trTag =
            <tr key={index} id={item.id}>
                ...
                <td align='center' onClick={ClickItem}>{hangul}({item.category})</td>
            </tr>
            ;
        return trTag;
    }
    ...
    const ClickButtonGroup = (event) => {
        ...
        const columns = ['name', 'price', 'category', 'category_kr'] ;        
    ...
    const ChangedItem = (event) => {
        ...
        // 선택한 option의 영문 이름을 넘겨 줍니다.
        onChangeCategory(selectedCategory); 
    }

    // 카테고리 목록을 반복하여 변수로 만들고, 콤보 박스에 동적으로 추가하기
                        ...
                        <td width="10%" valign="middle">
                            <Dropdown>
                                ...
                                <Dropdown.Menu>
                                    ...
                                    <Dropdown.Item id="category" onClick={ClickButtonGroup}>카테고리(영문)</Dropdown.Item>

                                    <Dropdown.Item id="category_kr" onClick={ClickButtonGroup}>카테고리(한글)</Dropdown.Item>
                                </Dropdown.Menu>
                            </Dropdown>
                        </td>
                        <td valign="middle">
